...
# Removed: from dotenv import load_dotenv - Environment variables should be set in App Engine config (app.yaml)
...
def initialize_rag():
    ...
    try:
        # The API key is fetched directly from the App Engine environment variables
        gemini_api_key = os.environ.get('GEMINI_API_KEY')
        if not gemini_api_key:
            # Note: For production deployment via GitHub, ensure GEMINI_API_KEY is 
            # securely set in the Cloud Console environment variables for the App Engine service.
            logging.warning("GEMINI_API_KEY environment variable not set. RAG will fail.")
    ...
    try:
        # Note: HuggingFaceEmbeddings will download the model weights to the local file system
        # during the App Engine build step (if not already cached).
